...
(define tests
  (test-suite
   ...
   (check-equal? (eval-exp (ifgreater (int 9) (int 4) (int 3) (int 2))) (int 3) "ifgreater test")

   ;; mlet test
   (check-equal? (eval-exp (mlet "x" (int 1) (add (int 5) (var "x")))) (int 6) "mlet test")
   (check-equal? (eval-exp (mlet "x" (add (int 1) (int 5)) (add (int 5) (var "x")))) (int 11) "mlet test")

   ;; call test
   (check-equal? (eval-exp (call (closure '() (fun #f "x" (add (var "x") (int 7)))) (int 1))) (int 8) "call test")
   (check-equal? (eval-exp (call (closure (cons (cons "y" (int 6)) empty) (fun "my-fn" "x" (add (var "x") (var "y"))))
                                 (int 1))) (int 7) "call test")

   ;;snd test
   (check-equal? (eval-exp (snd (apair (int 1) (int 2)))) (int 2) "snd test")
   (check-equal? (eval-exp (snd (apair (int 1) (add (int 2) (int 5))))) (int 7) "snd test")

   (check-equal? (eval-exp (fst (apair (int 1) (int 2)))) (int 1) "fst test")
   (check-equal? (eval-exp (fst (apair (add (int 2) (int 5)) (int 1)))) (int 7) "fst test")
